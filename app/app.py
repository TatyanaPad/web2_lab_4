...
def create_user():
    ...
    if message and password_message is not None:
        if message == 'Логин удовлетворяет всем требованиям' and password_message == 'Пароль удовлетворяет всем требованиям':
            ...
            return render_template('users_new.html', user=params, roles_list=load_roles(), input_class=input_class,
                                   div_class=div_class, message=message, password_input_class=password_input_class,
                                   password_div_class=password_div_class, password_message=password_message)
...
def change_password(user_id):
    ...
    return render_template('change_password.html', user_id=user_id, message=message, input_class=input_class,
                           div_class=div_class, confirm_message=confirm_message,
                           confirm_input_class=confirm_input_class, confirm_div_class=confirm_div_class,
